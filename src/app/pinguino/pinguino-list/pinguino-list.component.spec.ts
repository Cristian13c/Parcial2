...
describe('PinguinoListComponent', () => {
  ...
  it('should have 10 <id> elements', () => {
    expect(debug.queryAll(By.css('id'))).toHaveSize(10)
  });
  ...
  it('should have the corresponding src to the book image and alt to the pinguino name', () => {
    debug.queryAll(By.css('id')).forEach((img, i)=>{
      expect(img.attributes['src']).toEqual(
        component.pinguinos[i].id)


      expect(img.attributes['alt']).toEqual(
        component.pinguinos[i].name.toString())
    })
  });


  it('should have h5 tag with the pinguino.', () => {
    debug.queryAll(By.css('h5.card-title')).forEach((h5, i)=>{
      expect(h5.nativeElement.textContent).toContain(component.pinguino[i].edad)
    });
  });
